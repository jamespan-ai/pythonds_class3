...
## 3.顯示記錄期間即開始至結束
print('資料期間',sessions['session_start'].min(),'至',sessions['session_start'].max())
...
print(pd.to_numeric(sessions['device'],errors='coerce'))

## 8.字串清理,利用.str 存取器
## 8-1 將 campaign 欄位的內容 去頭尾空白,輸出為 temp3.csv
sessions['campaign_strip']=sessions['campaign'].str.strip()
sessions.to_csv('temp3.csv')

## 8-2 將 campaign 欄位的內容 去頭尾空白,輸出為 temp3.csv
sessions['campaign_replace']=sessions['campaign'].str.replace(' ','')
sessions.to_csv('temp3.csv')

## 8-3 查詢 campaign 欄位是否有包含 retarget 的值 ,輸出為 temp3.csv
sessions['is_retarget']=sessions['campaign'].str.contains('retarget')
sessions.to_csv('temp3.csv')

## 8-4 查詢 符合 campaign 為 retarget 的記錄,只看 campaign 與 device　二欄位
print(sessions[sessions['is_retarget']].head())
# 語法：df.loc[列條件, [欄位清單]]
print(sessions.loc[sessions['is_retarget'],['campaign','device']].head())
print('='*30)
## 8-5 檢查 campaign 欄位是否有異常的類別 利用 unique 方法
print(sessions['campaign'].unique())
